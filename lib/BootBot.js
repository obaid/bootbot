...
class BootBot extends EventEmitter {
  constructor(options) {
    ...
    logger.add(logger.transports.Console, { level: (options.debug ? 'debug' : 'info'), colorize: 'all' });
  ...
  start(port) {
    ...
    this.server = this.app.listen(this.app.get('port'), () => {
      ...
      logger.info('BootBot running on port', portNum);
      logger.info(`Facebook Webhook running on localhost:${portNum}/webhook`);
    });
  }
  ...
  sendTypingIndicator(recipientId, milliseconds) {
    ...
    if (milliseconds > 20000) {
      milliseconds = 20000;
      logger.error('sendTypingIndicator: max milliseconds value is 20000 (20 seconds)');
    }
  ...
  getUserProfile(userId) {
    ...
    return fetch(url)
      .then(res => res.json())
      .catch(err => logger.error(`Error getting user profile: ${err}`));
  }
  ...
  say(recipientId, message, options) {
    ...
    logger.error('Invalid format for .say() message.');
  }
  ...
  conversation(recipientId, factory) {
    if (!recipientId || !factory || typeof factory !== 'function') {
      return logger.error(`You need to specify a recipient and a callback to start a conversation`);
    }
